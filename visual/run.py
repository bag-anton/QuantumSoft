...
RESET_URL = 'http://127.0.0.1:8000/tree/reset'
CACHE_URL = 'http://127.0.0.1:8000/tree/cache'
DB_URL = 'http://127.0.0.1:8000/tree/db'
...
class Ui(QtWidgets.QMainWindow, Form):
    ...
    def get_item_from_db(self):
        index = (
            self.treeWidget_2.selectedIndexes()[0]
            if len(self.treeWidget_2.selectedIndexes())
            else None
        )
        if index:
    ...
    def fill_item(self, item, value):
        ...
        node = MyQTreeItem(
            value['value'], value['is_deleted'], value['id'], value['parent_id']
        )
        if value['is_deleted']:
            node.setForeground(0, QtGui.QBrush(QtGui.QColor('#FF0000')))
        item.addChild(node)
